  ...
  methods: {
    ...
    getWarInstType () { 
      ...
      this.$http({
        ...
        url: this.$http.adornUrl('/getJqlxtj?dateType=' + this.btnList[this.dateTypeBtnIdx].value),
        method: 'get',
      }).then(res => {
        if (res.data.code == '200') {
          // console.log("获取警情总量",res.data.data);
          this.jqTypeListc_2=[];
          self.jqTypeListc_2_tb_hb_list=[];
          let resData = []
          ...
          for (var i=0;i<data.length;i++) {
            ...
            resData.push({name: data[i].lxmc, value: data[i].num})
           // name: '处警量', tb: '4.71', hb: '0.71', tbStatusIsDown: true, hbStatusIsDown: false}
            self.jqTypeListc_2_tb_hb_list.push({name:data[i].lxmc,tb:data[i].tb>=100||data[i].tb<=-100?Number(data[i].tb).toFixed(0):Number(data[i].tb).toFixed(2),hb:data[i].hb>=100||data[i].hb<=-100?Number(data[i].hb).toFixed(0):Number(data[i].hb).toFixed(2),tbStatusIsDown:data[i].tb>0,hbStatusIsDown: data[i].tb<0});
          }
